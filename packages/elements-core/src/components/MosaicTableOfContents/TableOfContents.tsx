...
const TagGroupItem = ({ item, maxDepthOpenByDefault, onLinkClick }: TagGroupItemProps) => (
  <Box>
    <div className="sl-text-sm sl-leading-relaxed sl-tracking-wide sl-font-bold sl-uppercase sl-mt-6 sl-mb-2 sl-pl-4">
      {' '}
      {item.title}{' '}
    </div>
    {item.items.map((i, k) => (
      <GroupItem key={k} item={i} depth={1} maxDepthOpenByDefault={maxDepthOpenByDefault} onLinkClick={onLinkClick} />
    ))}
  </Box>
);
